...
for iBeat = 1:length(IOI)
    ...
    if iBeat <= length(IOI)
        ...
        if PB == 1
            Action = input('Do you want to replace beat [1], add beat [2], remove beat [3], or do nothing [0] ?');
            if Action == 0
            elseif Action == 1
                ...
                plot(beatOnset(mIndex), Audio(beatOnset(mIndex)), 'w*')
                beatOnset(mIndex) = [];
