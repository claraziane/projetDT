    ...
    for iSession = 1%:length(Sessions)
        ...
        for iCondition = 2:length(Conditions)
            ...
            for iStep = 1:length(stepOnsets)
                figure;
                subplot(2,2,1); plot(copLatFrontFilt(stepOnsets(iStep):stepOnsets(iStep+1)-1));
                subplot(2,2,2); plot(copLatRearFilt(stepOnsets(iStep):stepOnsets(iStep+1)-1));
                subplot(2,2,3); plot(copForwardFrontFilt(stepOnsets(iStep):stepOnsets(iStep+1)-1));
